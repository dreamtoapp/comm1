...
import { Shift } from '@prisma/client'; // Added import for Shift type
...
async function seedShifts(): Promise<{ dayShift: Shift; nightShift: Shift }> {
  try {
  ...
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      do {
        ...
        attempts++;
        if (attempts > 100) {
          throw new Error('Failed to generate a unique phone number after 100 attempts.');
        }
...
async function seedOrders(count: number, dayShift: Shift, nightShift: Shift): Promise<void> {
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const order = await db.order.create({
        data: {
          ...
          driverId: driver.id,
          status: faker.helpers.arrayElement(['Pending', 'Delivered', 'InWay', 'canceled']),
          amount: totalAmount,
