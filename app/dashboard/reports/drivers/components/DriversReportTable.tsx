          ...
          <TableBody>
            ...
            {drivers.map((driver) => {
              ...
              const completedOrders =
                driver.orders?.filter((o) => o.status === 'Completed').length || 0;
              const cancelledOrders =
                driver.orders?.filter((o) => o.status === 'Cancelled').length || 0;
              const totalEarnings =
                driver.orders?.reduce(
                  (sum, o) => sum + (o.status === 'Completed' ? o.amount : 0),
                  0,
                ) || 0;
              return (
                <TableRow key={driver.id}>
                  ...
                  <TableCell>
                    {totalEarnings.toLocaleString('ar-EG', { minimumFractionDigits: 2 })} ر.س
                  </TableCell>
                  <TableCell>
                    <a
                      href={`/dashboard/reports/drivers/${driver.id}`}
