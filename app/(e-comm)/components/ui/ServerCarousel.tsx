...
interface ServerCarouselProps {
  // Define props based on what this Server Component *receives*
  images: { id: string; url: string; alt: string }[]; // Expects 'url'
  className?: string;
}
...
export default async function ServerCarousel({ images, className }: ServerCarouselProps) {
  ...
  const mappedImages = images.map(img => ({
    ...
    alt: img.alt,
    // width/height can be omitted if SimpleCarousel handles defaults
  }));
