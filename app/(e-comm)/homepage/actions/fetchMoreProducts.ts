  ...
  try {
    ...
    return products.map((product) => {
      ...
      const processedProduct: Product = { // Explicitly map all fields
        ...
        details: product.details, // Already nullable in Product type
        size: product.size, // Already nullable in Product type
        published: product.published,
        outOfStock: product.outOfStock,
        inStock: !product.outOfStock, // Derive inStock from outOfStock
        // Always return a string for imageUrl
        imageUrl: hasValidImageUrl ? product.imageUrl : fallbackImage,
        images: product.images || [hasValidImageUrl ? product.imageUrl : fallbackImage],
        type: 'product', // Assuming 'product' is the correct default type here
        rating: product.rating,
        reviewCount: product.reviewCount,
        supplier: product.supplier, // Pass the selected supplier object
